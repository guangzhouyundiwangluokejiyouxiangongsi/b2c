...
<head>
<meta charset="utf-8">
<title>促销活动-<?php echo ($tpshop_config['shop_info_store_title']); ?></title>
<meta http-equiv="keywords" content="<?php echo ($tpshop_config['shop_info_store_keyword']); ?>" />
<meta name="description" content="<?php echo ($tpshop_config['shop_info_store_desc']); ?>" />
<link rel="stylesheet" href="/Template/pc/soubao/Static/css/promoteList.css" type="text/css">
...
<div class="dircontent">
    <div class="sort container">
      <div class="berserk clearfix">
		<script>
        // 倒计时
        function GetRTime2(){
            //var text = GetRTime('2016/02/27 17:34:00');
           <?php if(is_array($goodsList)): foreach($goodsList as $k=>$v): ?>var text<?php echo ($v[goods_id]); ?> = GetRTime('<?php echo (date("Y/m/d H:i:s",$v["end_time"])); ?>');								
            $("#surplus_text<?php echo ($v[goods_id]); ?>").text(text<?php echo ($v[goods_id]); ?>);<?php endforeach; endif; ?>
        }
        setInterval(GetRTime2,1000);
    </script>
      <?php if(is_array($goodsList)): foreach($goodsList as $k=>$v): ?><div class="berserk-item scale" id="zxmp_32268" data-fixsite="1" data-activityid="32268" isstarted="true" data-starttime="1456365600000" data-endtime="1456797599000" data-couponid="321844" data-total-count="1" data-total-page="4" data-current-page="1">
          <div class="berserk-pic"> <a href="<?php echo U('/Home/Goods/goodsInfo',array('id'=>$v[goods_id]));?>" target="_blank"> 
          <img src="<?php echo (goods_thum_images($v["goods_id"],250,250)); ?>"  class="scale-img"> </a>
            <div class="time">
              <div class="time-bg"> <i class="icon sprite"></i> </div>
              <div class="time-txt">&nbsp;&nbsp;剩余：<i class="sprite" id="surplus_text<?php echo ($v[goods_id]); ?>"></i></div>
            </div>
          </div>
          <div class="berserk-info info">
            <div class="info-txt">
              <h4 class="discountstr"><strong><?php echo round($v[price] / $v[shop_price] * 10 , 1); ?></strong>折起</h4>
              <p class="saleNum"><?php echo ($v[click_count]); ?>人已浏览</p>
            </div>
            <div class="info-logo"> 
              <a href="javascript:void(0);" target="_blank">
              <p class="title"><?php echo ($v[goods_name]); ?></p>
              <p class="info"> 商品原价￥<?php echo ($v[shop_price]); ?> </p>
              </a> 
            </div>
          </div>
        </div><?php endforeach; endif; ?>
